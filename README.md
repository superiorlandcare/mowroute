...
**Phase 8 — PWA polish** (done): MowRoute is an installable PWA (spec §14.5,
§15.8, §16). A web app manifest (`app/manifest.ts` → `/manifest.webmanifest`:
name, brand green theme color, `display: standalone`, generated 192/512 +
maskable icons + Apple touch icon) plus `appleWebApp` metadata make it
add-to-home-screen-ready on iOS and Android. A service worker (`public/sw.js`)
handles installability and a **graceful offline state**: navigations are
network-first and fall back to a branded `/offline.html` when the connection
drops (the crew works in low-signal areas), and immutable static assets are
cached. It deliberately **does not** cache authenticated pages or
Supabase/realtime traffic, so auth, role gating, dollar-stripping, and live sync
behave exactly as online and no stale private data is ever served. Icons are
generated by `scripts/gen-icons.mjs` (no dependencies). No schema change. This is
the last phase in §15.

**Map + route builder** (done): a full-screen **`/map`** view (crew + admin)
showing every active, geocoded customer as a big tap-friendly pin on free
Leaflet + OpenStreetMap tiles — tap a pin for name/address/services and
Google/Apple/Waze directions; an "N unmapped" chip lists customers with no
coordinates yet. The Setup customer form now embeds a **draggable-pin map**:
tap the map or drag the pin to set the exact location (with "Find address" as a
geocoded starting guess, kept even when low-confidence since the admin corrects
it by eye), and any hand-placed pin saves as a manual location that geocoding
never overwrites. Admins also get **Build Route** mode on the map: tapping pins
selects properties in order (green + numbered badge), a bottom bar tracks the
count, date (defaults to today), and the route's **start/end** (the shop by
default; override via address search, dropping a pin, or tapping a property),
and **Create Route** writes it through the existing model — a pending visit per
chosen service for that date's cycle (cadence-due services; if none are due,
the Mow — never an off-cadence extra that could bill early), the soft `day`
label retagged to that weekday, tap order permuted into the services' own
`sort_order` slots (the optimizer's trick), and the start/end saved to the new
`route_plans` table (`0004_route_plans.sql`), which the per-day Optimize button
now prefers over the env depot. The board additionally shows any service with a
visit this cycle even when not cadence-due, so map-built stops appear. No route
optimization changes beyond the start/end — selection and saving only.
...
| `supabase/migrations/0004_route_plans.sql` | Adds `route_plans` (custom start/end per route date; run after 0003). |
| `components/leaflet-map.tsx` | Shared Leaflet shell (free OSM tiles, client-only load) + divIcon pin builders. |
| `app/map/` | Full-screen property map + admin Build Route mode (`actions.ts` = `previewGeocode`, `createMapRoute`). |
| `app/setup/pin-picker.tsx` | Draggable-pin location picker embedded in the customer form. |
| `lib/data/map.ts` | Server-side map load: active customers + light (price-free) service summaries. |
| `lib/route-plan.ts` | Pure route-builder logic: service picking + tap-order → sort_order slots (tested by `scripts/test-route-select.mts`). |
| `lib/map-links.ts` | Google/Apple/Waze turn-by-turn deep links (shared by board + map). |
...
| `public/sw.js`, `app/sw-register.tsx` | Service worker (installability + offline fallback) + its registrar. |
| `public/offline.html`, `scripts/gen-icons.mjs` | Branded offline page; dependency-free PWA icon generator. |
